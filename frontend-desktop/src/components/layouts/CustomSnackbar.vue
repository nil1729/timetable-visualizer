<template>
	<div>
		...
		>
			<template v-slot:default>
				<slot :message="snackbar.message">
					<v-icon left>{{ typeIconMapper[snackbar.type] }}</v-icon>
			...
			<template v-slot:action>
				...
				>
					<v-btn text @click="removeMessage(snackbar.key, true)">
		...
		<css-style :key="key + idx" v-for="(key, idx) in keys">
			.v-snackbars.v-snackbars-{{ identifier }}-{{ key }} .v-snack__wrapper { transition:
			{{ topOrBottom[key] }} 500ms; {{ topOrBottom[key] }}: 0; } .v-snackbars.v-snackbars-{{
				identifier
			}}-{{ key }} > .v-snack__wrapper { {{ topOrBottom[key] }}:{{ calcDistance(key) }}px; }
		</css-style>
	</div>
</template>
