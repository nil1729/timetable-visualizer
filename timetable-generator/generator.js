...
const generateTimetableAPI = (preferences) => {
	...
	const generateTimetables = (numberOfCourses, courseCombinations, courseList, i, currentTT) => {
			...
			if (res && timetables.length <= numberOfTTs - 1) {
				...
				for (let j = 0; j < numberOfCourses; j++) {
					...
					indices[j].forEach((section) => {
						if (section[0] === 'L') {
							newTT.lecturesSection.instructors = courseDetails.SECTIONS[section].instructors;
							newTT.lecturesSection.section = section;
							newTT.lecturesSection.dhString = courseDetails.SECTIONS[section].timingString;
							...
							timings.forEach((e) => {
								...
								timeSlot.time =
									timings1[0].split('-')[0] + '-' + timings1[timings1.length - 1].split('-')[1];
								timingArray.push(timeSlot);
							});
							newTT.lecturesSection.timings = timingArray;
						} else if (section[0] === 'T') {
							newTT.tutorialsSection.instructors = courseDetails.SECTIONS[section].instructors;
							newTT.tutorialsSection.section = section;
							newTT.tutorialsSection.dhString = courseDetails.SECTIONS[section].timingString;
							let timings = courseDetails.SECTIONS[section].timings;
							...
							newTT.labsSection.instructors = courseDetails.SECTIONS[section].instructors;
							newTT.labsSection.section = section;
							newTT.labsSection.dhString = courseDetails.SECTIONS[section].timingString;
							let timings = courseDetails.SECTIONS[section].timings;
							let timingArray = [];
							timings.forEach((e) => {
								let timeSlot = new Timing();
		...
		for (let j = 0; j < courseCombinations[i].length && execute; j++) {
				...
				if (
					...
					for (let l = 0; l < timings.length && toBeAdded; l++) {
						...
						for (let m = 0; m < timings[l].timing.length; m++) {
							if (
								currentTT[parseInt(timings[l].timing[m]) - 1][daysMapping[timings[l].day]] !== ''
							) {
								clash = true;
								break;
							}
			...
			if (toBeAdded) {
				generateTimetables(numberOfCourses, courseCombinations, courseList, i + 1, currentTT);
			}
			...
			updatedCourseDetails.LECTURES = Object.keys(updatedCourseDetails.LECTURES)
				.filter((key) => {
					if (preferences[i][course_codes[i]].PREFERRED.LECTURES.length > 0) {
						return preferences[i][course_codes[i]].PREFERRED.LECTURES.indexOf(key) >= 0;
					}

					return preferences[i][course_codes[i]].UNPREFERRED.LECTURES.indexOf(key) < 0;
				})
				.reduce((obj, key) => {
					obj[key] = updatedCourseDetails.LECTURES[key];
					return obj;
				}, {});
			updatedCourseDetails.TUTORIALS = Object.keys(updatedCourseDetails.TUTORIALS)
				.filter((key) => {
					if (preferences[i][course_codes[i]].PREFERRED.TUTORIALS.length > 0) {
						return preferences[i][course_codes[i]].PREFERRED.TUTORIALS.indexOf(key) >= 0;
					}

					return preferences[i][course_codes[i]].UNPREFERRED.TUTORIALS.indexOf(key) < 0;
				})
				.reduce((obj, key) => {
			...
			updatedCourseDetails.LABS = Object.keys(updatedCourseDetails.LABS)
				.filter((key) => {
					if (preferences[i][course_codes[i]].PREFERRED.LABS.length > 0) {
						return preferences[i][course_codes[i]].PREFERRED.LABS.indexOf(key) >= 0;
					}

					return preferences[i][course_codes[i]].UNPREFERRED.LABS.indexOf(key) < 0;
				})
				.reduce((obj, key) => {
	...
	parsedTTs.scheduledCourses.sort(() => Math.random() - 0.5);
	return parsedTTs;
};
