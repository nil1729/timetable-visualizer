  ...
  TTs.forEach((TT_courses, index) => {
    TT_courses.forEach((TT) => {
      ...
      for (let j = 0; j < lecTimings.length; j++) {
        array2D[index][timesMapping[lecTimings[j].time]][
          daysMap[lecTimings[j].dayCode]
        ] = {
      ...
      for (let j = 0; j < tutTimings.length; j++) {
        array2D[index][timesMapping[tutTimings[j].time]][
          daysMap[tutTimings[j].dayCode]
        ] = {
          courseCode: TT.courseCode,
      ...
      for (let j = 0; j < labsTimings.length; j++) {
        ...
        let updatedTimings = [];
        let diff = eval(
          array_timing[1].split(":")[0] - array_timing[0].split(":")[0]
        );
        let first =
          timesMapping[
            array_timing[0].split(":")[0] +
              ":00" +
              " - " +
              array_timing[0].split(":")[0] +
              ":50"
