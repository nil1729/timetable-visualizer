	...
	const generateICS = (scheduledCourses) => {
		...
		scheduledCourses.forEach((course) => {
			...
			if (typeof tuts !== 'undefined') {
				strResult = common(tuts.timings, strResult, course, tuts.section, tuts.instructors);
			}
			const labs = course.labsSection;
			if (typeof labs !== 'undefined') {
				strResult = common(labs.timings, strResult, course, labs.section, labs.instructors);
			}
		});
	};
	generateICS(scheduledCourses);
	strResult += 'END:VCALENDAR';
	return fs.writeFileSync(path.join(__dirname, `../tmp/${filename}.ics`), strResult);
	...
	function common(timings, strResult, course, section, instructors) {
		timings.forEach((time) => {
			...
			strResult +=
				...
				semEnd.toISOString().split('-').join('').split(':').join('').split('.')[0] +
				'Z' +
				'\n';
			strResult +=
				'DTSTAMP:' +
				new Date().toISOString().split('.')[0].split(':').join('').split('-').join('') +
				'Z' +
				'\n';
			strResult +=
				'SUMMARY:' +
